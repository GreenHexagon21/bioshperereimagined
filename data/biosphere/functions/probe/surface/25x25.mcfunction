...
execute at @s if dimension minecraft:overworld in minecraft:overworld run setblock ~-12 ~-14 ~-12 structure_block[mode=save]{name:"biosphere:25x25_temp",posX:0,posY:1,posZ:0,sizeX:25,sizeY:25,sizeZ:25,rotation:"NONE",mirror:"NONE",mode:"SAVE",ignoreEntities:0b} replace
...
execute as @e[name="25x25_sur_ref",sort=random,limit=1] in biosphere:biosphere_dim at @s run function biosphere:probe/surface/25x25_ref